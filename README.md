...
In this hands-on lab you'll deploy a Virtual Power Plant (VPP) monitoring dashboard — built with Next.js — onto **Snowflake App Runtime** (Public Preview). You'll go from cloning the repo to a live app in about 15 minutes, without touching Docker, provisioning infrastructure, or managing credentials.

### Dashboard

Once deployed, the app monitors a simulated fleet of ~4,050 home battery systems and displays:
...
Before you can deploy apps, Snowflake needs to know **where to put them** on your account. This is configured via a one-time **App Development Setup** wizard in Snowsight.
...
Still as `ACCOUNTADMIN`, run the following grants. If you chose a deploy role other than `SYSADMIN` in Step 1, replace accordingly:
...
Next, you'll connect this Git repository to Snowsight as a Workspace. This gives you immediate access to all files — notebooks, CSV data, and app source code — right inside Snowsight, with no manual file uploads.
...
Now let's load the demo data. Open `notebooks/setup_data.ipynb` in your Workspace and click **Run All**. The notebook takes care of everything:
...
The `snow app` CLI commands need a local copy of the project. If you haven't already, clone the repo to your machine:
...
> **Important:** Make sure your Snowflake CLI connection uses the **deploy role** from Step 1 (e.g., `SYSADMIN`). You can verify with `snow connection status` — the `role` field should match.
...
The first deploy takes 3-5 minutes (uploads code, builds remotely, provisions the service). Subsequent deploys are faster (~2 min) thanks to layer caching.
...
This opens your app's live HTTPS URL in the browser. You'll authenticate with your normal Snowflake login and see the VPP Monitor dashboard.

---

## Maintaining the App

Once your app is running, here's how to manage it.
...
## Background: Snowflake App Runtime

This section explains the technology behind what you just deployed.
...
## About the Data: Virtual Power Plants

Here's the business context behind the dashboard.
...
├── README.md                       # This file
├── app.yml                         # App metadata (title, description, icon)
├── cleanup.sql                     # Full cleanup script
